...
?>
@if(Route::currentRouteName() !== 'user.profile.index')
            ...
            <div class="user-profile-info">
                <div class="info-new">
                    ...
                    <p class="text-10 mb-0">
                        {{ __("Member Since :time", ["time" => date("M Y", strtotime($dataUser->created_at))]) }}</p>
                </div>
            </div>
            @if(!Auth::user()->hasPermission("dashboard_vendor_access") and setting_item('vendor_enable'))
                <div class="user__profile-plan mt-10 text-center">
                    <a class="become-vendor button -sm -dark-1 bg-blue-1 text-white"
                        href="{{ route("user.upgrade_vendor") }}">{{ __("Become a vendor") }}</a>
                </div>
            @endif
        </div>
                                ...
                                <div class="sidebar__button {{$menuItem['class']}} col-12 d-flex items-center justify-between">
                                    <div class="d-flex items-center text-15 lh-1 fw-500">
                                        @if(!empty($menuItem['icon']))
                                            <a href="{{ url($menuItem['url']) }}" class="icon text-center mr-15 text-24"><i
                                                    class="{{$menuItem['icon']}}"></i></a>
                                        @endif
